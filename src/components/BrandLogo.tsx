...
// Linear's actual mark, provided by the user. Other brands fall back to
// colored initials below since we don't have their logo assets - mixing a
// real logo with guessed/approximated ones for the rest would be worse than
// consistent initials.
export function LinearMark({ className }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" fill="currentColor" className={className} aria-label="Linear">
...
export function BrandLogo({ brand, className }: BrandLogoProps) {
  ...
  return <span className={className}>{brand.slice(0, 2).toUpperCase()}</span>;
}
